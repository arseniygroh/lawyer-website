(() => {
    ...
    da.init();
    const titles = document.querySelectorAll(".top-footer__title");
    const mediaQuery = window.matchMedia("(max-width: 480px)");
    function accordion(media) {
        if (media.matches) for (let i = 0; i < titles.length; i++) titles[i].addEventListener("click", (function() {
            let list = this.nextElementSibling;
            if (list.style.display === "block") list.style.display = "none"; else list.style.display = "block";
        }));
    }
    accordion(mediaQuery);
